...
const makeTestLevel = (i) => {
  ...
  const factory = new LevelFactory(`${i}`, width, height)
    ...
    .addExits([
      new ExitTrigger(
        Rectangle.widthForm(-32, baseY, 32, 18),
        `${i - 1}`,
        i === 2 ? Rectangle.widthForm(-32, baseY - 18, 32, 36) : undefined
      ),
      new ExitTrigger(
        Rectangle.widthForm(32, baseY, 32, 18),
        `${i + 1}`,
        i === 0 ? Rectangle.widthForm(32, baseY - 18, 32, 36) : undefined
      ),
    ]);
  if (i === 0) {
    ...
    factory.addInteractibles([
      new PuzzleInteractible(
        "1",
        new Vector(5, baseY + 15),
        Rectangle.widthForm(3, baseY + 13, 4, 4)
      ),
    ]);
    factory.addObjects([
    ...
    factory.addInteractibles([
      new PuzzleInteractible(
        "3",
        new Vector(26, baseY + 13),
        Rectangle.widthForm(24, baseY + 11, 4, 4)
      ),
    ]);
  } else if (i === 5) {
